...
public:
				       ...
				       GaugeStats GaugeStatisticsCalculator=GaugeStats())
  {
    ...
    // Allow for malformatted files generated by older Grid versions
    } else if ( header.data_type == std::string("4D_SU"+stNC+"_GAUGE_"+stNC+"x"+stNC) || header.data_type == std::string("4D_SU3_GAUGE_3x3") ) {
      if ( ieee32 || ieee32big ) {
    ...
    // Allow for incorrectly normalised link traces
    assert(fabs(clone.link_trace-header.link_trace) < 1.0e-6 || fabs(clone.link_trace - (header.link_trace * 3) / Nc) < 1.0e-6);
